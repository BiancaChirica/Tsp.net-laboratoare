...
namespace MyPhotosGUI
{
    public partial class Form1 : Form
    {
        ...
        private void dataGrid_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex > -1)
            {
                string path = this.dtGridPhotos[0, e.RowIndex].Value.ToString();
                var listph = apiPhoto.GetPhotosByPath(path);
                if (listph == null)
                {
                    msgLabel.Text = "Can't select row.";
                    return;
                }
                SelectedPhotoPath = path;
                SelectedPhotoid = listph[0].PhotoId;
        ...
        private void Open_File_Click(object sender, EventArgs e)
        {
            ...
            {
                msgLabel.Text = "Path not valid.";
                apiPhoto.MarkDeleted(SelectedPhotoid);
                return;
            }

            var extension = Path.GetExtension(SelectedPhotoPath);
            ...
            {
                PlayForm pf = new PlayForm(@"C:\Users\bianc\OneDrive\Desktop\1.wmv.wmv");
                pf.ShowDialog();
            }
            else
            //img
            pictureBox1.Image = new Bitmap(SelectedPhotoPath);
