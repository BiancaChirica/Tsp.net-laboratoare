...
namespace MyPhotos
{
    public class ApiPhoto
    {
        ...
        public String DeletePhoto(int id)
        {
            using (ctx = new ModelBDContainer())
            {
                ...
                try
                {
                    ...
                    var obj = ctx.Photos.Where(k => k.PhotoId == id).SingleOrDefault();
                    if (obj == null)
                    {
                        return "Can't find file.";
                    }
                    ...
                    foreach (Proprietate item in prop.ToList())
                    {
        ...
        public Photo GetPhotosById(int id)
        {
            using (ctx = new ModelBDContainer())
            {
                return ctx.Photos.Where(p => p.PhotoId == id).ToList().FirstOrDefault();
            }
        }
